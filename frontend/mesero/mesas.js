...
// ===== Mesas: data de ejemplo (reemplazar por fetch a tu API) =====
const ESTADOS = [
  { value: 'libre',        label: 'Libre',        cls: 'bg-green-100 text-green-700' },
  { value: 'ocupada',      label: 'Ocupada',      cls: 'bg-yellow-100 text-yellow-700' },
  { value: 'en-preparacion', label: 'En preparación', cls: 'bg-blue-100 text-blue-700' },
  { value: 'bloqueada',    label: 'Bloqueada',    cls: 'bg-gray-200 text-gray-600' },
];

const mesas = [
  { id: 1, estado: 'libre', editable: true },
  { id: 2, estado: 'ocupada', editable: true },
  { id: 3, estado: 'en-preparacion', editable: true },
  { id: 4, estado: 'bloqueada', editable: false }, // ejemplo: no editable (cuenta abierta, cierre en curso, etc.)
];
...
function renderFila(mesa){
  ...
  const canEdit = !!mesa.editable;

  return `
    <tr data-id="${mesa.id}">
      <td class="px-6 py-4 font-semibold">Mesa ${mesa.id}</td>
  ...
  tbody.querySelectorAll('tr').forEach(tr => {
    ...
    if (btnGuardar && btnCancelar){
      btnGuardar.addEventListener('click', async () => {
        ...
        // TODO: Llamar a tu API (PUT /mesas/:id) para persistir
        // await fetch(`/api/mesas/${id}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ estado: nuevo }) });
